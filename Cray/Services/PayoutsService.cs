...
namespace Cray.Services
{
    public class PayoutsService : BaseService
    {
        ...
        {
            return await _client.GetAsync<object>($"payout/payment-methods/{countryCode}");
        }

        public async Task<object> Banks(string countryCode = null)
        {
            var url = "payout/banks";
        ...
        {
            return await _client.PostAsync<object>("payout/accounts/validate", data);
        }

        public async Task<object> Disburse(object data)
        {
            return await _client.PostAsync<object>("payout/disburse", data);
        }

        public async Task<object> Verify(string transactionId)
        {
            return await _client.GetAsync<object>($"payout/requery/{transactionId}");
        }
    }
}
